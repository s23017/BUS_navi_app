...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '112.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_112_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha112_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs112 = idxJson && idxJson['112'] && idxJson['112'][cal];
        if (!Array.isArray(recs112) || recs112.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs112 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha112_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha112_1,${cal},${tripId},112`);
    ...
    console.error('[build_naha_112_gtfs] failed to apply naha_time_index_all to 112:', e);
  }
  ...
  console.log('[build_naha_112_gtfs] wrote GTFS-like files into public/naha_112 (with S-id mapping where possible)');
}
