...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '11down.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_11down_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha11down_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs120 = idxJson && idxJson['11down'] && idxJson['11down'][cal];
        if (!Array.isArray(recs120) || recs120.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs120 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha11down_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha11down_1,${cal},${tripId},11down`);
    ...
    console.error('[build_naha_11down_gtfs] failed to apply naha_time_index_all to 11down:', e);
  }
  ...
  console.log('[build_naha_11down_gtfs] wrote GTFS-like files into public/naha_11down (with S-id mapping where possible)');
}
