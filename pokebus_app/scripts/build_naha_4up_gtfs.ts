...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '4up.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_4up_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha4up_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      // derive route key from source filename (e.g. '4up')
      const routeKey = path.basename(srcPath, '.json');

      function pickIndexRecords(idx: any, baseKey: string, cal: string) {
        if (!idx) return null;
        const candidates = [
          baseKey,
          // strip trailing 'up'/'down' (e.g. '4up' -> '4')
          baseKey.replace(/(up|down)$/i, ''),
          // keep only digits (e.g. '4up' -> '4')
          (baseKey.match(/\d+/) || [''])[0],
        ];
        for (const k of candidates) {
          if (!k) continue;
          const bucket = idx[k] && idx[k][cal];
      ...
      for (const cal of calendars) {
        const recs4up = pickIndexRecords(idxJson, routeKey, cal);
        if (!Array.isArray(recs4up) || recs4up.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs4up as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha4up_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha4up_1,${cal},${tripId},4up`);
    ...
    console.error('[build_naha_4up_gtfs] failed to apply naha_time_index_all to 4up:', e);
  }
  ...
  console.log('[build_naha_4up_gtfs] wrote GTFS-like files into public/naha_4up (with S-id mapping where possible)');
}
