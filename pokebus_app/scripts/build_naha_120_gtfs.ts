...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '120.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_120_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha120_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs120 = idxJson && idxJson['120'] && idxJson['120'][cal];
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha120_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha120_1,${cal},${tripId},120`);
    ...
    console.error('[build_naha_120_gtfs] failed to apply naha_time_index_all to 120:', e);
  }
  ...
  console.log('[build_naha_120_gtfs] wrote GTFS-like files into public/naha_120 (with S-id mapping where possible)');
}
