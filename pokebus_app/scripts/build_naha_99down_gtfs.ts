...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '99down.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_99down_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha99down_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs99down = idxJson && idxJson['99down'] && idxJson['99down'][cal];
        if (!Array.isArray(recs99down) || recs99down.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs99down as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha99down_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha99down_1,${cal},${tripId},99down`);
    ...
    console.error('[build_naha_99down_gtfs] failed to apply naha_time_index_all to 99down:', e);
  }
  ...
  console.log('[build_naha_99down_gtfs] wrote GTFS-like files into public/naha_99down (with S-id mapping where possible)');
}
