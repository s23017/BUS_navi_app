...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '12up.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_12up_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha12up_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs12up = idxJson && idxJson['12up'] && idxJson['12up'][cal];
        if (!Array.isArray(recs12up) || recs12up.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs12up as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha12up_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha12up_1,${cal},${tripId},12up`);
    ...
    console.error('[build_naha_12up_gtfs] failed to apply naha_time_index_all to 12up:', e);
  }
  ...
  console.log('[build_naha_12up_gtfs] wrote GTFS-like files into public/naha_12up (with S-id mapping where possible)');
}
