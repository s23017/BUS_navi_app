...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '7.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_7_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha7_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs7 = idxJson && idxJson['7'] && idxJson['7'][cal];
        if (!Array.isArray(recs7) || recs7.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs7 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha7_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha7_1,${cal},${tripId},7`);
    ...
    console.error('[build_naha_7_gtfs] failed to apply naha_time_index_all to 7:', e);
  }
  ...
  console.log('[build_naha_7_gtfs] wrote GTFS-like files into public/naha_7 (with S-id mapping where possible)');
}
