...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '12.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_12_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha12_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs12 = idxJson && idxJson['12'] && idxJson['12'][cal];
        if (!Array.isArray(recs12) || recs12.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs12 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha12_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha12_1,${cal},${tripId},12`);
    ...
    console.error('[build_naha_12_gtfs] failed to apply naha_time_index_all to 12:', e);
  }
  ...
  console.log('[build_naha_12_gtfs] wrote GTFS-like files into public/naha_12 (with S-id mapping where possible)');
}
