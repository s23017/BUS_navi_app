...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '55.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_55_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha55_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha55_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha55_1,${cal},${tripId},55`);
    ...
    console.error('[build_naha_55_gtfs] failed to apply naha_time_index_all to 55:', e);
  }
  ...
  console.log('[build_naha_55_gtfs] wrote GTFS-like files into public/naha_55 (with S-id mapping where possible)');
}
