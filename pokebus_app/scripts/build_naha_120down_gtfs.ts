...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '120down.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_120down_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('[build_naha_120down_gtfs] failed to parse JSON:', e);
    process.exit(1);
  }

  const route = data && Array.isArray(data.routes) ? data.routes[0] : null;
  if (!route || !Array.isArray(route.stops) || route.stops.length === 0) {
    console.error('[build_naha_120down_gtfs] no structured stops in 120down.json');
    process.exit(1);
  }

  const baseIndex = buildBaseStopIndex(publicRoot);

  const outDir = path.join(publicRoot, 'naha_120down');
  ensureDir(outDir);

  const routesLines = [
    'route_id,route_short_name,route_long_name,route_type,agency_id',
    `naha120down_1,120down,120down 名護西空港線,3,naha_bus`,
  ];
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha120down_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs120down = idxJson && idxJson['120down'] && idxJson['120down'][cal];
        if (!Array.isArray(recs120down) || recs120down.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs120down as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha120down_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha120down_1,${cal},${tripId},120down`);
    ...
    console.error('[build_naha_120down_gtfs] failed to apply naha_time_index_all to 120down:', e);
  }
  ...
  console.log('[build_naha_120down_gtfs] wrote GTFS-like files into public/naha_120down (with S-id mapping where possible)');
}
