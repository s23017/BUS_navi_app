...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '7up.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_7up_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha7up_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs7up = idxJson && idxJson['7up'] && idxJson['7up'][cal];
        if (!Array.isArray(recs7up) || recs7up.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs7up as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha7up_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha7up_1,${cal},${tripId},7up`);
    ...
    console.error('[build_naha_7up_gtfs] failed to apply naha_time_index_all to 7up:', e);
  }
  ...
  console.log('[build_naha_7up_gtfs] wrote GTFS-like files into public/naha_7up (with S-id mapping where possible)');
}
