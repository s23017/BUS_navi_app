...
function main() {
  ...
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_4_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha4_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs4 = idxJson && idxJson['4'] && idxJson['4'][cal];
        if (!Array.isArray(recs4) || recs4.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs4 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha4_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha4_1,${cal},${tripId},4`);
    ...
    console.error('[build_naha_4_gtfs] failed to apply naha_time_index_all to 4:', e);
  }
  ...
  console.log('[build_naha_4_gtfs] wrote GTFS-like files into public/naha_4 (with S-id mapping where possible)');
}
