...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '120up.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_120up_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha120up_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs120up = idxJson && idxJson['120up'] && idxJson['120up'][cal];
        if (!Array.isArray(recs120up) || recs120up.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs120up as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha120up_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha120up_1,${cal},${tripId},120up`);
    ...
    console.error('[build_naha_120up_gtfs] failed to apply naha_time_index_all to 120up:', e);
  }
  ...
  console.log('[build_naha_120up_gtfs] wrote GTFS-like files into public/naha_120up (with S-id mapping where possible)');
}
