...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '113.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_113_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha113_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs113 = idxJson && idxJson['113'] && idxJson['113'][cal];
        if (!Array.isArray(recs113) || recs113.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs113 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha113_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha113_1,${cal},${tripId},113`);
