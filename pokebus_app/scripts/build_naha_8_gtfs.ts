...
function main() {
  ...
  const srcPath = path.join(publicRoot, 'naha', '8.json');
  if (!fs.existsSync(srcPath)) {
    console.error('[build_naha_8_gtfs] source not found:', srcPath);
    process.exit(1);
  }
  ...
  for (const s of route.stops as any[]) {
    ...
    const stopId = match?.stop_id || `naha8_stop_${seq}`;
  ...
  try {
    ...
    if (fs.existsSync(idxPath)) {
      ...
      for (const cal of calendars) {
        const recs8 = idxJson && idxJson['8'] && idxJson['8'][cal];
        if (!Array.isArray(recs8) || recs8.length === 0) continue;

        // tripSid ごとにまとめる。stopName が入っているものを優先し、無ければ time 単位で。
        const byTrip: Map<string, { time: string }[]> = new Map();
        for (const r of recs8 as any[]) {
        ...
        for (const [tripKey, list] of byTrip.entries()) {
          ...
          const tripId = `naha8_trip_${tripCounter}`;
          tripCounter++;

          tripsLines.push(`naha8_1,${cal},${tripId},8`);
    ...
    console.error('[build_naha_8_gtfs] failed to apply naha_time_index_all to 8:', e);
  }
  ...
  console.log('[build_naha_8_gtfs] wrote GTFS-like files into public/naha_8 (with S-id mapping where possible)');
}
